...
If we wanted, for example, to get the total contributions from the top 20 contributors, we would have a list of 20 rows we want to match against. That's where our old friend <code>IN</code> comes to the rescue:
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/amount_by_state.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/amount_by_city_state.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/city_state_by_amount_desc.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/group_by_without_city.png!
...
Now that we understand grouping and aggregates, let's try filtering the results based on an aggregate. To start, let's find all cities for which the total contributions is greater than $3,000. Here's a first
stab at the query:
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/warning_aggregate_where.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/having_amount_greater.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/where_gt_2300.png!
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/where_gt_2300_with_group.png!
...
Now let's further filter this list of cities. We want to look only at cities in which these large contributions ($2,300 or greater) made a big difference. Let's call $4000 a big difference, for the sake of argument. So, we want only those where the total of contributions at this size exceeds $4000. (Looking at the results from the last query, we know to expect 3 rows, but it's not always so easy to see.) Here goes:
...
!http://github.com/tthibo/SQL-Tutorial/raw/master/where_plus_having.png!
