...
<b>9. Find all of the contributors to Hillary Clinton</b>
...
<pre><code>SELECT count(contributions.id), candidates.id, candidates.last_name FROM candidates LEFT OUTER JOIN contributoins ON candidates.id = contributions.candidate_id GROUP BY candidates.id, candidates.last_name;</code></pre>
